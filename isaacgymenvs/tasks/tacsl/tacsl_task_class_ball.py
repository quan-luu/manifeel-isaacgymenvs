...
from isaacgymenvs.tasks.tacsl.tacsl_env_ball import TacSLEnvBall
...
class TacSLTaskClassBall(TacSLTaskImageAugmentation, TacSLEnvBall, FactoryABCTask):
    ...
    def _reset_small_large_gears(self):
        ...
        random_offset_value = 0.1        
        random_y = torch.tensor(
            [random.choice([0.0, random_offset_value]) for _ in range(self.num_envs)],
            dtype=torch.float32, device=self.device
        )
        print("🏓First random_y:", random_y[:1].cpu().numpy())
        random_offset = torch.stack(
            [torch.zeros_like(random_y), random_y, torch.zeros_like(random_y)], dim=1
        )
    ...
    def _reset_medium_gear(self, before_move_to_grasp):
        ...
        if before_move_to_grasp:
            ...
            # Use a variable to control random offset value, easy to change in the future
            random_offset_value = -0.01

            # Compute medium gear offset based on small gear's y-axis random offset
            # If small gear's y offset is 0, medium gear gets 0.05; otherwise, it gets 0
            small_random_y = self.gear_small_pos[:, 1] - self.base_pos[:, 1]
            medium_random_y = random_offset_value - small_random_y
            medium_random_offset = torch.stack(
                [torch.zeros_like(medium_random_y), medium_random_y, torch.zeros_like(medium_random_y)],
                dim=1
            )
